...
class SlugPreviewWidget(widgets.TextInput):
    ...
    class Media:
        css = {
            "all": ("slug_preview/css/slug_preview.css",),
        }
        js = (
...
class JSONEditorWidget(forms.Widget):
    class Media:
        js = (getattr(settings, "JSON_EDITOR_JS", 'utils/dist/jsoneditor.min.js'),)
        css = {
            'all': (
                getattr(settings, "JSON_EDITOR_CSS", 'utils/dist/jsoneditor.min.css'),
            )
        }

    template_name = 'utils/django_json_widget.html'
